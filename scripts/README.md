...
(vllm) into the host's per-user secret store (Development only; never committed). When `.env`
carries `AZURE_AD_*` values (see below) it also writes the `AzureAd:*` and `SpaConfiguration:*`
secrets that enable REST API token validation and the admin SPA login. Then:

```powershell
dotnet run --project tests/Swarmwright.Example.WebHost   # browse https://localhost:7001
```

## Provisioning Entra ID auth (`provision-app-registration.ps1`)

The REST API validates Entra ID (Azure AD) bearer tokens and the admin SPA signs users in with
MSAL, both backed by a single app registration. Provision it (idempotently) against whatever tenant
your `az` session points at — it creates the app registration, exposes the `Swarm.Read` /
`Swarm.Write` scopes and the `Swarm.Admin` role, mints a 24-month client secret, and writes a
managed `AZURE_AD_*` block into `.env`:

```powershell
pwsh ./scripts/provision-app-registration.ps1
pwsh ./scripts/set-user-secrets.ps1   # then push AzureAd:* + SpaConfiguration:* into user-secrets
```

Requires the Azure CLI (`az login`) with rights to register applications. Re-running rotates the
secret cleanly. Without this, the host runs unauthenticated and the SPA login fails with
"tenant not found".
