...
Every factual claim MUST include provenance. Use these formats:

- **Web source:** `[Source Title](https://actual-url)` — the URL you fetched via `web_fetch`
- **MCP data:** `[MCP: provider/tool_name] "brief query or params"` — the tool call that produced the data
- **Model knowledge:** prefix with `[model-knowledge]` — flags the claim as unverifiable from this session's sources

Do NOT write vague attributions like "according to studies" or "research shows." Those are useless without a link. If you fetched a webpage, include the URL. If you queried a database, include the tool and query. If you cannot cite a specific source, use the `[model-knowledge]` tag so the synthesis can flag it as lower confidence.

Your `task_update(result=...)` MUST include a **Sources** section at the end listing every URL and MCP call you referenced, deduplicated.
