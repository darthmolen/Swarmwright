...
Every data point and statistical claim MUST include provenance. Use these formats:

- **Web source:** `[Source Title](https://actual-url)` — the URL you fetched via `web_fetch`
- **MCP data:** `[MCP: provider/tool_name] "query or params"` — include enough detail to reproduce the result (e.g., the SQL query, API parameters, date range)
- **Model knowledge:** prefix with `[model-knowledge]` — flags the claim as unverifiable from this session's sources

Numbers without provenance are noise. If you cite a metric, the reader must be able to trace it back to the exact source or query that produced it.

Your `task_update(result=...)` MUST include a **Sources** section at the end listing every URL and MCP call you referenced.
