...
describe('TaskPillBar', () => {
  ...
  it('applies correct color class for failed status', () => {
    const tasks: Task[] = [makeTask({ id: 'task-1', status: 'failed' })];
    render(<TaskPillBar tasks={tasks} selectedTaskId={null} onSelect={() => {}} />);
    expect(screen.getByTestId('task-pill-task-1').className).toContain('task-pill--failed');
  });
  ...
  // Split pill: status icon + name
  it('renders a status icon button that calls onNavigate with swarm_id', () => {
    const onNavigate = vi.fn();
    const tasks: Task[] = [
      makeTask({ id: 'task-1', swarm_id: 'swarm-abc', status: 'completed' }),
    ];
    render(<TaskPillBar tasks={tasks} selectedTaskId={null} onSelect={() => {}} onNavigate={onNavigate} />);

    const statusBtn = screen.getByTestId('task-status-task-1');
    fireEvent.click(statusBtn);
    expect(onNavigate).toHaveBeenCalledWith('swarm-abc');
  });

  it('status icon has the status color class', () => {
    const tasks: Task[] = [
      makeTask({ id: 'task-1', swarm_id: 'swarm-abc', status: 'completed' }),
    ];
    render(<TaskPillBar tasks={tasks} selectedTaskId={null} onSelect={() => {}} onNavigate={() => {}} />);

    const statusBtn = screen.getByTestId('task-status-task-1');
    expect(statusBtn.className).toContain('--completed');
  });

  it('does not render status icon when onNavigate is not provided', () => {
    const tasks: Task[] = [makeTask({ id: 'task-1' })];
    render(<TaskPillBar tasks={tasks} selectedTaskId={null} onSelect={() => {}} />);

    expect(screen.queryByTestId('task-status-task-1')).toBeNull();
  });
});
