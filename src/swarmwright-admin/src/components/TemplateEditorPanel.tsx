...
export function TemplateEditorPanel({
  ...
  const fetchTemplateFiles = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/swarm/templates/${templateKey}`, {
        headers,
      });
      if (!res.ok) {
        setFiles([]);
  ...
  }, [templateKey, workerName, getAuthHeaders]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/swarm/templates/${templateKey}/files/${selectedFile}`,
        {
          method: 'PUT',
          headers,
