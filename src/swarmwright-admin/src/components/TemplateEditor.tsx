...
export function TemplateEditor({ onClose }: TemplateEditorProps) {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    const res = await fetch(`${API_BASE}/api/swarm/templates`, { headers });
    if (res.ok) {
  ...
  async function handleSelectTemplate(key: string) {
    ...
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_BASE}/api/swarm/templates/${key}`, { headers });
    if (res.ok) {
      const data = await res.json();
      setFiles(data.files);
  ...
  async function handleSave() {
    ...
    const res = await fetch(
      `${API_BASE}/api/swarm/templates/${selectedKey}/files/${selectedFile}`,
      {
        method: 'PUT',
        headers,
  ...
  async function handleCreate() {
    ...
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_BASE}/api/swarm/templates`, {
      method: 'POST',
      headers,
  ...
  async function handleDelete() {
    ...
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_BASE}/api/swarm/templates/${selectedKey}`, {
      method: 'DELETE',
      headers,
    });
